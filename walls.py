...
SCENT_FALLOFF = 1.5
SHOW_SCENT = True
LAYER_NEURONS = 16  # hidden layer size
...
def ray_segment_intersection(rx, ry, rdx, rdy, x1, y1, x2, y2):
    ...
    # Segment: (x1, y1) + u*((x2-x1), (y2-y1)), 0 <= u <= 1
    ...
    return t  # distance along ray direction
...
class Brain(nn.Module):
    def __init__(self):
        super().__init__()
        input_size = 12 + VISION_SEGMENTS * 2
...
class Creature:
    ...
    def vision_input(self, food_list, walls, k_nearest=10):
        # --- FOOD VISION ---
        if not food_list:
            food_result = np.zeros(VISION_SEGMENTS, dtype=np.float32)
        else:
            dists = []
            sx, sy = self.x, self.y
        ...
        wall_result = np.zeros(VISION_SEGMENTS, dtype=np.float32)
        ...
        for i in range(VISION_SEGMENTS):
            ...
            min_t = None
            for w in walls:
                t = ray_segment_intersection(self.x, self.y, dx, dy, w.x1, w.y1, w.x2, w.y2)
                if t is not None and 0 < t < VISION_RANGE:
            ...
            if min_t is None:
                ...
                # Normalize: 1.0 = very close, 0.0 = far
                wall_result[i] = 1.0 - (min_t / VISION_RANGE)

        return food_result, wall_result


    def update(self, food_list, walls):
        if not food_list:
            return 0
        ...
        food_vision, wall_vision = self.vision_input(food_list, walls)
        inputs = np.concatenate((base_inputs, food_vision, wall_vision), dtype=np.float32)
        ...
        # Wall collision: if too close to any wall, revert and "bounce"
        for w in walls:
            d = point_segment_distance(self.x, self.y, w.x1, w.y1, w.x2, w.y2)
            if d < 8.0:
...
def run_generation(creatures, food, walls, generation):
    ...
    for step in range(GENERATION_STEPS):
        ...
        for c in creatures:
            if c is not picked_creature:
                food_eaten += c.update(food, walls)
    ...
    walls = spawn_walls(WALLS_COUNT)

    with open("simulation_log.json", "w") as f:
        json.dump(log_data, f, indent=2)

    return True, creatures, food, walls
...
while running:
    ...
    running, creatures, food, walls = run_generation(creatures, food, walls, generation)
